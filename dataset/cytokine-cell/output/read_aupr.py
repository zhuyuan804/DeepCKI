...
df = pd.DataFrame()
# columns = ["Without features","Only use sequence feature","Only use location feature","Only use domain feature","use location and domain features","use sequence and location features","use sequence and domain features","Use all the features",
#            "Use all features plus identity","Only use Cellular component feature","Only use  Biological process feature","Only use Molecular function feature","Use all the GO features","Use all the GO features and sequence feature"]

model = ['VAE','GAE','VGAE']

for i in model:
    path = save_path = os.path.join(i + "/results_graph2go_" + args.supervised + "_" +
                                 args.graph + "_" + str(1) + "_" + str(args.thr_ppi) + "_" + str(
                                     args.epochs_ppi))
    with open(save_path + "_5cytokine_cell.csv", "r") as f:
        file = pd.read_csv(f)
        file = file.set_index(['Unnamed: 0'])
        file = file.reindex(["M-aupr","M-auc","m-aupr","precision","recall","m-auc","F1-score"])
        roc = file['all']
...
df2.to_csv("evaluation index.csv")
